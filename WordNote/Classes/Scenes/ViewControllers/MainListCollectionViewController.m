...
- (void)addListDictionaryAction{
    ...
    [self.ListDictionary setObject:@"归档" forKey:k_File];
...
- (void)CollectionViewFlowLayoutAction{
    ...
    collectionViewFlowLayout.headerReferenceSize = CGSizeMake(self.view.frame.size.width, 20);
    //设置item大小
    collectionViewFlowLayout.itemSize = CGSizeMake(90, 90);
    //设置每列最小间距
    ...
    //注册headerView
    [self.collectionView registerClass:[LOCollectionRV class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"headerView"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.FileLabel.textColor = [UIColor blueColor];
...
//        NSLog(@"%d",longPressGr.view.tag);
        ...
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"是否要删除文件夹" message:str delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定",nil];
        alertView.tag = 3000;
        [alertView show];
    }
}

#pragma mark  - 设置header和footer
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    LOCollectionRV *reuseableView = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"headerView" forIndexPath:indexPath];
    
    if (indexPath.section == 0) {
        reuseableView.label.text = k_File;
    }else{
        reuseableView.label.text = k_CustomFile;
    }
    //返回
